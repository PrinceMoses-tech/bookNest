...
import { FiUser, FiMail, FiLock, FiBook, FiEye, FiEyeOff, FiAlertCircle, FiCheck } from 'react-icons/fi';
...
export default function SignupPage() {
  ...
  const pwStrength = form.password.length >= 8 ? 'strong' : form.password.length >= 6 ? 'medium' : form.password.length > 0 ? 'weak' : '';
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 relative overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative text-center">
          ...
          <h2 className="text-3xl font-bold text-white mb-3">Join BookNest</h2>
            ...
            {perks.map(p => (
              <li key={p} className="flex items-center gap-3 text-sm text-slate-300">
                <div className="w-5 h-5 rounded-full bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center shrink-0">
                  <FiCheck size={11} className="text-indigo-400" />
                </div>
                {p}
              </li>
            ))}
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-white dark:bg-slate-950">
        ...
        >
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100 mb-1">Create your account</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Join BookNest and start reading today</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPw(s => !s)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                  ...
                  {['weak', 'medium', 'strong'].map((level, i) => (
                    <div
                      ...
                      className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                        ...
                        (pwStrength === 'strong' && i <= 2) ? 'bg-emerald-400' :
